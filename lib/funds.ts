// Base de produits Kompa — extraite du prototype kompa-prototype.html
// Chaque entrée décrit un produit financier avec ses caractéristiques pédagogiques.

export type FundGeo = { c: string; p: number };
...
export function findFund(query: string): Fund | undefined {
  if (!query) return undefined;
  var q = query.trim().toLowerCase();
  if (!q) return undefined;
  return FUNDS.find(function (f) {
    return f.match.some(function (m) { return m.indexOf(q) !== -1 || q.indexOf(m) !== -1; });
  });
}
